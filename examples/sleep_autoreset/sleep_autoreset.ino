...
void loop()
{
	...
	Senseo.sleep(SENSEO_RISING, WAKE_UP, AUTO_RESET); // This line allows to enter in sleep mode with the following config : rising edge exit on D2 and on D3 with reset after exit
  //Senseo.sleep(SENSEO_FALLING, WAKE_UP_3, AUTO_RESET);  // This line allows to enter in sleep mode with the following config : rising edge exit only on D3 with reset after exit. 
                                                          // Note that this line should be used with the global macro SENSEO_SLEEP_3
